...
// go test -v decode_test.go decode.go encode.go codec.go

var (
	REPLY []byte = []byte{114, 1, 0}
)

func TestParseNull(t *testing.T) {
	// data = bytes.NewBuffer(append(REPLY, 'N'))
	// h := NewHessian(bytes.NewReader(data.Bytes()))
	h := NewHessian(append(REPLY, 'N'))
	v, err := h.Parse()
	if err != nil {
...
func TestParseInt32(t *testing.T) {
	...
	// h := NewHessian(bytes.NewReader(data.Bytes()))
	h := NewHessian(append(REPLY, []byte{'I', 0, 0, 0, 89}...))
	v, err := h.Parse()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestParseLong(t *testing.T) {
	...
	// h := NewHessian(bytes.NewReader(data.Bytes()))
	h := NewHessian(append(REPLY, []byte{'L', 0, 0, 0, 0, 1, 47, 129, 172}...))
	v, err := h.Parse()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestParseDate(t *testing.T) {
	...
	// h := NewHessian(bytes.NewReader(data.Bytes()))
	h := NewHessian(append(REPLY, []byte{'d', 0, 0, 1, 67, 206, 0, 226, 40}...))
	v, err := h.Parse()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestParseDouble(t *testing.T) {
	...
	// h := NewHessian(bytes.NewReader(data.Bytes()))
	h := NewHessian(append(REPLY, []byte{'D', 64, 159, 20, 61, 217, 127, 98, 183}...))
	v, err := h.Parse()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestParseString(t *testing.T) {
	...
	vs := v.(string)
	if vs != want {
		t.Fatalf("wnat %s(len:%d),but got %v(len:%d)", want, len(want), vs, len(vs))
	}
}
...
func TestParseListWithoutType(t *testing.T) {
	b := []byte{
...
func TestParseListWithType(t *testing.T) {
	b := []byte{
